...
def est_dans_rectangle(x, y):
    """Retourne True si le point de coordonnées (x,y) du canevas est dans le rectangel, et False sinon"""
    return 100 <= x <= 400 and 100 <= y <= 400


def chg_couleur(event):
    global cpt, continuer
    liste_couleur = ["blue", "red", "yellow"]
    if continuer:
        if est_dans_rectangle(event.x, event.y):
